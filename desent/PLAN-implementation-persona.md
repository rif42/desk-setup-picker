...
- 2026-07-10: Unloaded workspace from the opening flow. Removed the `done`-beat `RevealedRoom`/`WorkspacePreview` render, deleted `components/opening/RevealedRoom.tsx`, and dropped `previewVisible`. Flow is now arrival -> enter -> sharp `office.webp` reveal -> short hold -> blurred room + persona chips. `components/workspace/*` remain on disk, parked for the later builder wave. Re-verified green.
- 2026-07-10: Removed the reveal delay, the sharp-room hold, and the sharp `office.webp` beat. `OpeningExperience` now goes `preparing -> persona` directly after preload; reveal/hold timers and `REVEAL_DELAY_MS`/`PERSONA_HOLD_MS` deleted; the room mounts already blurred and chips show immediately. `revealing`/`done` remain in the `OpeningPhase` union (unused) for now. Re-verified green.
- 2026-07-10: Added an entering-the-door transition. Lifted `doors.webp` out of `ArrivalHero` into a new persistent `DoorBackdrop` (arrival/preparing hold, `revealing` forward dolly scale 1->1.24 + brightness bloom, then fade). Reintroduced a single `revealing` beat (`ENTERING_MS`) as the door-entry transition; `SlidingDoor`'s bloom/ring plays over it. The room (`office.webp`) mounts during `revealing` at opacity 0 and slowly fades in (blurred) at `persona`, crossfading with the door fade-out; chips appear after. Re-verified green.
- 2026-07-10: Fixed persona-stage click block — the `DoorBackdrop` wrapper (z-10) intercepted pointer events (inner `pointer-events-none` is not inherited). Added `pointer-events-none` to the wrapper.
- 2026-07-10: Persona chips now start with **no selection** (was Coder default). Selecting a chip advances to a new `builder` phase and **unblurs** `office.webp` (persistent layer, continuous sharp). `WorkspaceDesigner` mounts in the builder phase over the sharp room. Preset application (persona -> store) stays deferred — current `lib/catalog.ts` uses placeholder IDs, so applying a preset could empty the preview. Added `builder` to `OpeningPhase`, plus matching `DoorBackdrop`/`SlidingDoor` states. Re-verified green.
